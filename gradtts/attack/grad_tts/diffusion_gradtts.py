...
class Diffusion(BaseModule):
    ...
    def secmi_stat(self, dataloader, n_timesteps, batch_size=128, terminal_time=0.12, score_type='mse'):
        scores = []

        member_loader, nonmember_loader = load_member_data(dataset_name='ljspeech', batch_size=batch_size,
                                                           shuffle=False, randaugment=False)

        def recon_intermediate(n_timesteps, terminal_time, x0, mu, mask):
            intermediate_reverse = self.ddim_reverse(x0, mask, mu, n_timesteps, terminal_time)
            intermediate_denoise = self.ddim_denoise(intermediate_reverse, mask, mu, terminal_time)
            del intermediate_reverse[-1]
            del intermediate_denoise[0]
            return intermediate_reverse, intermediate_reverse

        for member, nonmember in zip(member_loader, nonmember_loader):
            member_x0, member_mu, member_mask = member
            nonmember_x0, nonmember_mu, nonmember_mask = nonmember

            member_intermediate_reverse, menber_intermidiate_denoise = recon_intermediate(n_timesteps, terminal_time, member_x0, member_mu, member_mask)
            nonmember_intermediate_reverse, nonmenber_intermidiate_denoise = recon_intermediate(n_timesteps, terminal_time, nonmember_x0, nonmember_mu, nonmember_mask)
