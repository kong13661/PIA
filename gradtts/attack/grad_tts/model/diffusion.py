...
class Diffusion(BaseModule):
    ...
    def ddim_denoise_2(self, xt_intermediate, mask, mu, terminal_time=0.12, spk=None):
        n_timesteps = len(xt_intermediate)
        h = terminal_time / n_timesteps
        intermediate_result = []
        for i, xt in enumerate(xt_intermediate):
            xt = xt * mask
            t = (((i + 1)) * h) * torch.ones(xt.shape[0], dtype=xt.dtype,
                                             device=xt.device)
            time = t.unsqueeze(-1).unsqueeze(-1)
            noise_t = get_noise(time, self.beta_min, self.beta_max,
                                cumulative=False)
            dxt = 0.5 * (mu - xt - self.estimator(xt, mask, mu, t, spk))
            dxt = dxt * noise_t * h * 0.5
            xt_half = (xt - dxt) * mask

            t_half = (((i + 1) - 0.5) * h) * torch.ones(xt.shape[0], dtype=xt.dtype,
                                                        device=xt.device)
            time_half = t_half.unsqueeze(-1).unsqueeze(-1)
            noise_t_half = get_noise(time_half, self.beta_min, self.beta_max,
                                     cumulative=False)
            dxt = 0.5 * (mu - xt_half - self.estimator(xt_half, mask, mu, t_half, spk))
            dxt = dxt * noise_t_half * h
            xt = (xt - dxt) * mask
    ...
    def ddim_reverse_2(self, x0, mask, mu, n_timesteps, terminal_time=0.12, spk=None):
        h = terminal_time / n_timesteps
        xt = x0 * mask
        ...
        for i in range(n_timesteps):
            t = ((i) * h) * torch.ones(x0.shape[0], dtype=x0.dtype,
                                       device=x0.device)
            time = t.unsqueeze(-1).unsqueeze(-1)
            noise_t = get_noise(time, self.beta_min, self.beta_max,
                                cumulative=False)
            ...
            t_half = ((i + 0.5) * h) * torch.ones(x0.shape[0], dtype=x0.dtype,
                                                  device=x0.device)
            time_half = t_half.unsqueeze(-1).unsqueeze(-1)
            noise_t_half = get_noise(time_half, self.beta_min, self.beta_max,
                                     cumulative=False)
    ...
    def secmi_stat_2(self, x0, mu, mask, n_timesteps, terminal_time=0.12):
        intermediate_reverse = self.ddim_reverse_2(x0, mask, mu, n_timesteps, terminal_time)
        intermediate_denoise = self.ddim_denoise_2(intermediate_reverse, mask, mu, terminal_time)
        del intermediate_reverse[-1]
        del intermediate_denoise[0]
        return intermediate_reverse, intermediate_denoise
